...
We used a set of 42 features in for the model. \autoref{feature} provides a brief description of features used in this experiment. The functions to calculate these features are implemented in `tsfeatures` R package by @tsfeatures. Most of the features have been previously used by @fforms and @hyndman2015large.  The last five heterogenity features are calculated as below. First, the time series is pre-whitened using an AR model to give a new series $z$. Then, a GARCH(1, 1) model is fitted to the pre-whited series and obtain the residual($r$) series. Subsequently, the features 39-42 are computed.  A detailed description of other features are provided in @fforms. 
