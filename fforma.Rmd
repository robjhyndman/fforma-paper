...
For each series $x_{new}$, we used as the centre of the interval the point forecast produced by our meta-learner. Then the 95% bounds of the interval were generated by a linear combination of the bounds of three forecasting methods: \naive, theta and seasonal \naive. \textcolor{red}{These methods were chosen from the initial *pool* for computational reasons, the whole *pool* should be used if computation time is not a constraint.} The coefficients for the linear combination were calculated in a data-driven way over the M4 database. The complete procedure was as follows:

1. We divided the M4 dataset into two parts: A and B. We trained the FFORMA learner using the training periods of the series in part A and produced point forecasts over the test periods of the series of part B, and vice versa. \textcolor{red}{This separation is required to prevent the overfitting effect caused from too accurate point forecasts if the same dataset used both for center and interval bounds.}

2. We computed the 95% *prediction interval radius* for the \naive, theta, and seasonal \naive\ methods. This is the difference between the 95% upper bound and the point forecast for each forecast horizon. \textcolor{red}{We assume the intervals to be symmetric around its center.}
...
In order to evaluate the impact of our contribution, we compared the average forecast error produced by FFORMA with a model selection approach. All implementation details were kept the same as in FFORMA; specifically we used the same set of features, the same pool of forecasting methods, the same underlying implementation (`xgboost`) but with a standard cross-entropy loss, and the same hyper-parameter search. This enabled us to measure the impact of the FFORMA loss function against a model selection approach, all other things being equal. We applied both FFORMA and the model selection approach to the M4 dataset and compared their overall point forecast errors. The average OWA error of the FFORMA approach was 10\% smaller than that for model selection. This improvement is entirely due to the proposed model averaging rather than model selection. \textcolor{red}{ On the other hand, FFORMA deviates significantly from simple averaging, the latter produces a 14\% increase in error for the same *pool* of methods. A simple cluster analysis of the weights produced by FFORMA in the M4 dataset shows that roughly 40\% of the time series receive a weight profile similar to equal weights (a simple average) while for the remaining 60\% of the series, one of the methods in the *pool* is clearly dominant. The performance of FFORMA is also robust against changes in the *pool*, the maximum increase in error when removing a single forecasting algorithm from the original 9 methods *pool* is 1\%. This maximum occurs when removing the Random Walk with Drift algorithm, that receives an average of 15\% weight in the original M4. Also, removing every single method results in increase in error compared to the original *pool*. Therefore, there are no methods in the original pool that produce a negative impact, even though individually some methods perform much worse than others when averaged over all the series in the M4 dataset. This suggests that FFORMA is able to assign correct weights to forecasting algorithms that are specialized to a specific types of time series.
Therefore, the good performance of FFORMA can be attributed to the two  following effects:
1) The weights produced by FFORMA can range from simple averaging (equal weights) to a profile that assigns most of the importance to only a single method depending on the series. This makes FFORMA outperfom both simple averaging and method selection alternatives.
2) FFORMA adapts to the methods in its pool, making the specific pool of methods less critical than in other combination approaches.}
