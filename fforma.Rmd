...
\textcolor{red}{The frequency of the series is taken from the M4 Competition data. \textcolor{blue}{I don't think we need to add this. The referee comment is inappropriate.}}
...
\textcolor{red}{This separation is required to prevent the overfitting effect caused from too accurate point forecasts if the same dataset used both for
center and interval bounds.} \textcolor{blue}{This partitioning prevents overfitting caused from having point forecasts that are extremely accurate if the same dataset was used for both center and the interval bounds.}

2. We computed the 95% *prediction interval radius* for the \naive, theta, and seasonal \naive\ methods. This is the difference between the 95% upper or lower bound and the point forecast for each forecast horizon,  \textcolor{red}{as we assume the intervals to be symmetric around the point forecast.}
...
\textcolor{red}{On the other hand, FFORMA deviates significantly from simple averaging. The latter produces a 14\% increase in error for the same *pool* \textcolor{blue}{(Not sure why we need *pool* in the new red text. I guess we can delete all those).} of methods. A simple cluster analysis of the weights produced by FFORMA in the M4 dataset shows that roughly 40\% of the time series receive a weight profile similar to equal weights (a simple average) while for the remaining 60\% of the series, one of the methods in the *pool* is clearly dominant.} 

\textcolor{red}{The performance of FFORMA is also robust against changes in the *pool* of forecasting methods. The maximum increase in error when removing a single method from the original set of nine methods is 1\%. This maximum occurs when removing the Random Walk with Drift, which receives an average of 15\% weight in the original M4. \textcolor{blue}{(Do we need "original M4" here? Should we just say 15\% weight across all the series in the M4? Is this what this means here?)} Also, the removal of each method results in an increased error compared to the original *pool*. Hence, there are no methods in the pool that impact the error negatively, even though individually some methods perform much worse than others when averaged over all the series in the M4 dataset. This suggests that the FFORMA algorithm is able to assign correct weights to the forecasting methods that are specialized for a specific type of series.} 

\textcolor{red}{Therefore, we believe that the good performance of the FFORMA can be attributed to: (i) the weights allocated by FFORMA can range from a simple averaging (equal weights), to a profile that assigns most of the importance to only a single method. This allows FFORMA to outperfom both simple averaging and method selection alternatives; (ii) FFORMA adapts to the methods in its pool, making the specific pool of methods less critical than in other combination approaches.}
