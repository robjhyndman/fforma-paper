...
One advantage of our approach is that its form is independent of the forecasting loss  measure. Forecasting errors enter the model as additional pre-calculated values. This allows FFORMA to adapt to arbitrary loss functions when models that directly minimize them would be restricted, e.g. our approach can be applied to non-differentiable errors.
