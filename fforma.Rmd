...
In order to evaluate the impact of our contribution, we compared the average forecast error produced by FFORMA with a model selection approach. All implementation details were kept the same as in FFORMA; specifically we used the same set of features, the same pool of forecasting methods, the same underlying implementation (`xgboost`) and the same hyper-parameter search. This enabled us to measure the impact of the unique FFORMA loss function coupled with a model selection approach, all other things being equal. We applied both FFORMA and the model selection approach to the M4 dataset and compared their overall point forecast errors. The average OWA error of the FFORMA approach was 10\% smaller than that for model selection. This improvement is entirely due to model averaging rather than model selection.
