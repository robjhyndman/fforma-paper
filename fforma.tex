...
forecast combination (\textcite{Aiolfi2011-yz}). Simple
averages(ref\_clemen1989), regression-based approaches, \ldots{} to name
a few.
...
We used a set of 42 features in for the model. \autoref{feature}
provides a brief description of features used in this experiment. The
functions to calculate these features are implemented in
\texttt{tsfeatures} R package by \textcite{tsfeatures}. Most of the
features have been previously used by \textcite{fforms} and
\textcite{hyndman2015large}. The last five heterogenity features are
calculated as below. First, the time series is pre-whitened using an AR
model to give a new series \(z\). Then, a GARCH(1, 1) model is fitted to
the pre-whited series and obtain the residual(\(r\)) series.
Subsequently, the features 39-42 are computed. A detailed description of
other features are provided in \textcite{fforms}.
